...
> Removal pass: walk statelist unlinking and freeing cells (and their states) whose mergeable==2. Latent bug: when the deleted cell is the list head the code assigns `statelist = s` (the cell being freed) instead of s->next, so the head points at freed memory; it usually still works because the freed cell's fields are read before being reused.
...
> 1. Root: scan statelist from the head, incrementing smax per cell; the first state whose lexstate name is "Root" gets number 0, cell start=1, n=1, and the scan stops (so smax = that cell's 1-based position, not necessarily the state count — see bug below). If there is no Root lexicon, continue a fresh scan to the LAST cell (= the first state ever created = the first lexicon mentioned in the file), give it number 0 and start=1 (n=1), and if g_verbose print to stderr (fflushed): `*Warning: no Root lexicon, using '%s' as Root.\n` with that lexicon's name.
...
> Latent bug: "#" is intended to get the highest number (statecount-1), but smax is Root's position in the list = 1 + number of states created after Root's state; this equals the total count only when Root's state was the very first state created (Root is the first lexicon name encountered). Otherwise "#" can receive a number that collides with one assigned sequentially in step 3, corrupting the number-indexed array built by `[spec:foma:sem:lexcread.lexc-to-fsm-fn]`.
...
> No bounds check on intarr: callers pass the static cwordin/cwordout buffers (1000 ints), so an entry side of 1000+ tokens overflows them (latent bug). Malformed UTF-8 (utf8skip == -1) makes skip+1 == 0, so the copy yields an empty string and i never advances — infinite loop on invalid input (latent bug).
