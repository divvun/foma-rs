...
> Inserts one of the three special symbols by numeric code, keeping the list sorted by number. Maps code→string: EPSILON=0→strdup("@_EPSILON_SYMBOL_@"), IDENTITY=2→strdup("@_IDENTITY_SYMBOL_@"), UNKNOWN=1→strdup("@_UNKNOWN_SYMBOL_@"); any other code leaves the string NULL (latent bug: a node with NULL symbol is then inserted).
